...
// problem: https://codeforces.com/problemset/problem/1832/B
...
using node =
    std::tuple<std::uint64_t, std::int32_t, std::int32_t, std::int32_t>;  // NOTE: {high priority, low priority }
}  // namespace px
...
void solution() {
  ...
  std::vector<std::uint64_t> prefix_sum(n + 1, 0);
  for (int i = 0; i < n; ++i) prefix_sum[i + 1] = prefix_sum[i] + a[i];

  std::uint64_t result = 0;
  for (int i = 0; i <= k; ++i) result = std::max(result, prefix_sum[n - (k - i)] - prefix_sum[2 * i]);

...
// problem: https://codeforces.com/contest/2037/problem/D
...
void solution() {
  ...
  std::priority_queue<int> nodes;
  int power = 1;
  int u = 0;
  int count = 0;

  for (int i = 0; i < n; i++) {
    int block = a[i].second - a[i].first + 1;

    while (u < m && b[u].first < a[i].first) {
      nodes.push(b[u].second);
      u += 1;
    }

    while (!nodes.empty() && power < block + 1) {
      power += nodes.top();
      nodes.pop();
      count += 1;
    }

...
// problem: https://codeforces.com/contest/2028/problem/0
...
void solution() {
  ...
  std::unordered_map<char, px::node> directions = {{'N', {0, 1}}, {'E', {1, 0}}, {'S', {0, -1}}, {'W', {-1, 0}}};

  int x = 0;
  int y = 0;
  std::string reached = "NO";

  int k = 20 * 20 * 10;  // (10, 10, 10, 10): grid(NEWS) * 10(max string size each time)

  while (k--) {
    for (int idx = 0; idx < n; ++idx) {
      auto [dx, dy] = directions[pre[idx]];
      x += dx;
      y += dy;

...
void solution() {
  ...
  std::uint64_t sum_even = 0;
  std::uint64_t even_count = 0;

  std::uint64_t sum_odd = 0;
  ...
  std::uint64_t even = sum_even % even_count;
  std::uint64_t odd = sum_odd % odd_count;
  if (!even && !odd && (sum_even / even_count == sum_odd / odd_count)) {
    std::cout << "YES" << '\n';
  } else {
    std::cout << "NO" << '\n';
  }

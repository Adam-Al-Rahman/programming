...
// problem: https://codeforces.com/problemset/problem/1966/B
...
void solution() {
  ...
  if (grid[0][0] != grid[n - 1][m - 1]) {
    ...
    for (int i = 1; i < m; i++) {
      if (grid[0][i - 1] != grid[0][i] || grid[n - 1][i - 1] != grid[n - 1][i]) is_correct = false;
    }

    if (is_correct) ans = "NO";

    is_correct = true;
    for (int i = 1; i < n; i++) {
      if (grid[i - 1][0] != grid[i][0] || grid[i - 1][m - 1] != grid[i][m - 1]) is_correct = false;
    }

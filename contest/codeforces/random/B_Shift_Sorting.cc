...
// problem: https://codeforces.com/problemset/problem/1969/B
...
void solution() {
  ...
  std::int64_t cost = 0;
  std::queue<int> zero_positions;

  for (int i = 0; i < bin.size(); ++i) {
    if (bin[i] == '0') zero_positions.push(i);
  }

  for (int i = 0; i < bin.size(); ++i) {
    if (bin[i] == '1' && !zero_positions.empty() && zero_positions.front() > i) {
      int j = zero_positions.front();  // Get the nearest '0' index

      std::swap(bin[i], bin[j]);
      cost += (j - i + 1);
    }

    if (!zero_positions.empty()) zero_positions.pop();
  }

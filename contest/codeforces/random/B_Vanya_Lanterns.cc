...
void solution() {
  ...
  double ans = std::max(a[0], l - a[n - 1]);
  for (int i = 1; i < n; i++) ans = std::max(ans, (a[i] - a[i - 1]) / 2);

  // Consider the largest gap, and the distance from the first and last lanterns to the edges

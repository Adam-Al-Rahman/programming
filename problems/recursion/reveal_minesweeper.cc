...
// squares, with hidden "mines" scattered throughout the board. The objective is to clear the board
// without detonating any mines, with help from clues about the number of neighboring mines in
// each field." Specifically, when a player clicks on a square (also called a cell) that doesn't contain a
// mine, the square reveals a number representing the number of immediately adjacent mines
// (including diagonally adjacent mines).

// You're given a two-dimensional array of strings that represents a Minesweeper board for a game
// in progress. You're also given a row and a column representing the indices of the next square
// that the player clicks on the board. Write a function that returns an updated board after the click
...
// • "0-8" : A cell with no mine, with an integer from O to 8 representing the number of
// adjacent mines. Note that this is a single-digit integer represented as a string. For example
// "2" would mean there are 2 adjacent cells with mines. Numbered cells are not clickable
// as they have already been revealed.

// If the player clicks on a mine, replace the "M" with "X" , indicating the game was lost.

// If the player clicks on a cell adjacent to a mine, replace the "H" with a string representing the
// number of adjacent mines.

// If the player clicks on a cell with no adjacent mines, replace the "H" with "O" . Additionally,

...
std::vector<std::vector<std::int32_t>> get_permutations(
    std::vector<std::int32_t> list, std::uint32_t index = 0) {
...
// use deque
std::vector<std::vector<std::int32_t>> permutations(
  ...
  for (int i = 0; i < list.size(); i++) {
    ...
    std::vector<std::vector<std::int32_t>> current_permutation =
        permutations(list, current_result);
    current_result.pop_back();
    list.insert(list.begin() + i, element);
    permutation.insert(permutation.end(), current_permutation.begin(),
